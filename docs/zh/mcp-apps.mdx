...
WorldMonitor 通过 `io.modelcontextprotocol/ui` 扩展支持 [MCP Apps](https://modelcontextprotocol.io/extensions/apps/build)。当前阵容提供 10 个 MCP Apps：自包含的 `ui://` HTML 资源，MCP Apps 宿主可在关联的工具调用后内联渲染它们。
...
| `ui://worldmonitor/natural-disasters.html` | `get_natural_disasters` | 自然灾害（交互式） | 近期地震（USGS 震级、地点、时间）和活跃野火（NASA FIRMS），分组展示。 |
