---
title: "命令行客户端"
description: "使用官方 worldmonitor npm CLI 直接从 shell 脚本化调用国家简报、风险评分、监视列表与全部 63 个 MCP 工具，无需编写任何自定义 API 集成代码即可完成自动化，非常适合运维脚本、CI/CD 流水线、本地情报工作流与临时数据分析。"
---
...
你会收到全部 63 个工具定义，格式为 JSON。其他所有返回*数据*的操作都是 `tools/call`，需要用户 API 密钥。
...
精选表格刻意保持精简——全部 63 个工具中的每一个都可通过 `call` 访问，任何未识别的 `--key value` 都会成为工具参数，因此无需任何特殊接线：
...
- **[MCP 工具参考](/zh/mcp-tools-reference)**——全部 63 个工具的逐工具参数、新鲜度预算和示例。
