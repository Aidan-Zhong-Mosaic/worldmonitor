---
title: "国家韧性指数"
description: "面向 196 个国家、包含 72 个指标的实时韧性评分体系，覆盖 21 个活跃维度与 6 大领域 —— 一个每 6 小时刷新一次的 0-100 综合指数，采用 OECD、JRC 等国际对等标准进行同侪基准比较，用于衡量国家治理、经济、社会与安全韧性的相对强弱、动态变化与结构性差距。"
---
...
对于实时部署状态，请使用[运行时清单](https://api.worldmonitor.app/resilience/v1/get-runtime-manifest)。它报告活动公式标签、静态 `dataVersion`、排名缓存元数据和安全派生构造版本：`constructVersions.energy`（`legacy` 或 `v2`）与 `constructVersions.education`（`active` 或 `rollback`）。它还报告公共 `scoreInterval`/`rankStable` 支持路径的 `intervals` 可用性元数据，而不暴露部署标识符、原始 `RESILIENCE_*` 标志状态或内部缓存键。参考版仍是冻结的已发布包；不应将其视为当前生产中活动运行时状态的证据。

以下文档描述**当前发布**状态：72 个指标覆盖 21 个活跃维度与 6 个领域，schemaVersion `"2.0"` 形状，3 支柱（加上 2 个结构性退役维度为模式连续性在注册表中保留，`coverage=0`），支柱组合惩罚 `overall_score`，以及能源 v2 构造。[支柱组合分数激活](#pillar-combined-score-activation-active) 小节记录了激活证据和回退机制。2026-06-02 实时运行时清单报告 `formulaTag="pc"` 和 `constructVersions.energy="v2"`，`/api/health` 对三个必需的能源 v2 种子检查（`lowCarbonGeneration`、`fossilElectricityShare` 和 `powerLosses`）报告 `OK`。
...
WorldMonitor 国家韧性指数以 0-100 分对 196 国公开可排名范围评分，使用 72 个指标覆盖 21 个活跃维度和 6 个领域（加上 2 个结构性退役维度为模式连续性在注册表中以 `coverage=0` 保留）。排名处理器仍可将低置信度或标题不合格的国家路由到 `greyedOut[]`，但可排名范围本身由提交的 UN 成员 + SAR 白名单固定。它将结构基线指标（治理质量、健康基础设施、财政能力）与实时压力信号（网络威胁、冲突事件、航运中断）和恢复能力指标（财政空间、储备、进口集中度）结合，产出每 6 小时更新的单一韧性分数。
...
操作步骤见 [`financial-system-exposure-flag-flip-runbook.md`](./financial-system-exposure-flag-flip-runbook.md)；量测结果见已提交的 [`resilience-financial-system-exposure-acceptance-2026-08-13.json`](../snapshots/resilience-financial-system-exposure-acceptance-2026-08-13.json) 快照。
