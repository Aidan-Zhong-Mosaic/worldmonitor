...
## Aircraft Tracking (adsb.lol + OpenSky)

Military aircraft are tracked via multiple ADS-B networks. The primary source is **adsb.lol** (`api.adsb.lol/v2/mil`), which returns military-filtered aircraft globally with no API key required. **Wingbits** can supplement the global response in its configured regions. OpenSky is fully disabled in the centralized military-flight seeder, so every cron cycle consumes zero metered OpenSky credits. Optional point-query gap-fill from **airplanes.live** and **adsb.fi** is disabled by default because both public APIs restrict use to non-commercial deployments; an eligible operator must explicitly enable it with `WM_ENABLE_NONCOMMERCIAL_ADSB_GAP_FILL=1`.

**Sources**:

- adsb.lol — primary, keyless, ODbL-licensed
- Wingbits — keyed regional supplement
- airplanes.live — optional non-commercial point-query gap-fill (1 request/second)
- adsb.fi — optional personal/non-commercial point-query gap-fill (1 request/second)
- OpenSky Network — legacy per-request service recovery only; disabled in the seeder
...
|------------|-----|---------|
| **Active cache** | 5 minutes | Matches the military-flight seeder cadence |
| **Stale cache** | 1 hour | Fallback when upstream APIs fail |

This ensures consistent data across all users while keeping provider calls in the centralized seeder.
