/**
 * Major international oil and gas pipeline reference data.
 * Sources: Global Energy Monitor, EIA, public domain geographic data.
 *
 * Lives in shared/ (no src/ imports) so server-side analysis — the
 * infrastructure-cascade core and the tools built on it — can bundle it under
 * Edge/esbuild. src/config/pipelines.ts re-exports this array typed as the
 * client-facing `Pipeline` and keeps the DOM/theme colour helpers.
 */

export type PipelineTypeName = 'oil' | 'gas' | 'products';
export type PipelineStatusName = 'operating' | 'construction';

export interface PipelineTerminalRecord {
  country: string;
  name?: string;
  portId?: string;
  lat?: number;
...
export interface PipelineRecord {
  ...
  operator?: string;
  countries?: string[];
  origin?: PipelineTerminalRecord;
  destination?: PipelineTerminalRecord;
