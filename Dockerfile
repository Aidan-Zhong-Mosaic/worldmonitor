...
# generate:mosaic first so the LOB CSVs, not a stale generated.ts, decide panels
RUN npm run generate:mosaic && npm run build:crawlable-corpus && npm run build:sitemap && npx tsc && npx vite build
...
COPY docker/nginx-mosaic-allowlist.conf /etc/nginx/mosaic-allowlist.conf
